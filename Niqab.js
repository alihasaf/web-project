...
function sortNiqabs(niqabs, sort) {
  switch (sort) {
    case "price_asc": return [...niqabs].sort((a,b) => a.price-b.price);
    case "price_desc": return [...niqabs].sort((a,b) => b.price-a.price);
    case "color_asc": return [...niqabs].sort((a,b) => a.color.localeCompare(b.color));
    case "color_desc": return [...niqabs].sort((a,b) => b.color.localeCompare(a.color));
    default: return niqabs;
  }
}

const Niqab = () => {
  const NIQABS = React.useMemo(() => COLORS.map((color, i) => ({
    id: i+1,
    color,
    price: Math.floor(Math.random() * 501) + 500, // 500-1000
  ...
  const sorted = sortNiqabs(NIQABS, sort);
  ...
  return (
    ...
    <div>
      <div className="flex flex-col items-center pt-8 pb-2">
        <video src="https://www.w3schools.com/html/mov_bbb.mp4" autoPlay loop muted className="w-full max-w-4xl h-[520px] rounded-lg shadow mb-4 object-cover" />
        <h2 className="text-3xl font-bold text-pink-700 mb-2 tracking-wide text-center">AVAILABLE NIQABS</h2>
        <div className="text-md text-gray-600 mb-2 text-center">STUFF: GEORGETTE NIQAB, LAWN NIQAB</div>
...
<button onClick={()=>handleAddToWishlist({
  ...
  quantity: 1
            ...
            <div className="relative w-80 h-96 lg:w-96 lg:h-[440px] mb-2">
              <img src={d.img} alt={d.color} className="w-full h-full object-cover rounded border" onError={e=>e.target.src="https://via.placeholder.com/320x384?text=Niqab"} />
            </div>
